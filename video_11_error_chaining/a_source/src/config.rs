...
mod tests {
    ...
    fn test_missing_config_returns_domain_error() {
        let result = load_config("non_existent_file.toml");
        assert!(matches!(
            result,
            Err(ConfigError::FileError(ref e)) if e.kind() == ErrorKind::NotFound
        ));
    }
    ...
    fn config_error_io_exposes_underlying_source() {
        ...
        let source = err
            .source()
            .expect("ConfigError::Io must return a source error");
